...
try {
  ...
  await visit("/");
  await page.getByRole("button", { name: "Open menu" }).click();
  await page.getByRole("navigation", { name: "Main navigation" }).getByRole("link", { name: "Your Reserve" }).focus();
  ...
  await page.getByRole("link", { name: /Begin your Reserve intake/ }).click();
  ...
  assert.ok(await page.getByRole("heading", { name: "Save your Reserve intake." }).isVisible());
  await page.getByRole("button", { name: "Open My Sanctum preview" }).click();
  await page.waitForURL("**/my-sanctum");
  await page.getByText("Your consultation Blueprint is saved to your Reserve account.").waitFor();
